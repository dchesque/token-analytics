...
EXECUTIVE SUMMARY
-----------------
Overall Score: {result.get('fundamental', {}).get('score', 0)}/10
Classification: {result.get('fundamental', {}).get('classification', 'N/A')}
Decision: {result.get('fundamental', {}).get('decision', 'N/A')}
Completion: {result.get('completion_rate', 0)}%

FUNDAMENTAL ANALYSIS
--------------------
Market Cap: ${result.get('fundamental', {}).get('token_data', {}).get('market_cap', 0):,.0f}
Volume 24h: ${result.get('fundamental', {}).get('token_data', {}).get('volume_24h', 0):,.0f}
Price Change 24h: {result.get('fundamental', {}).get('token_data', {}).get('price_change_24h', 0):.2f}%

Score Breakdown:
- Market Position: {result.get('fundamental', {}).get('scoring_categories', {}).get('market_position', {}).get('score', 0)}/2
- Liquidity: {result.get('fundamental', {}).get('scoring_categories', {}).get('liquidity', {}).get('score', 0)}/2
- Community: {result.get('fundamental', {}).get('scoring_categories', {}).get('community', {}).get('score', 0)}/2
- Development: {result.get('fundamental', {}).get('scoring_categories', {}).get('development', {}).get('score', 0)}/2
- Performance: {result.get('fundamental', {}).get('scoring_categories', {}).get('performance', {}).get('score', 0)}/2

TECHNICAL ANALYSIS
------------------
Trend: {result.get('technical', {}).get('trend', {}).get('trend_strength', 'N/A')}
RSI: {result.get('technical', {}).get('momentum', {}).get('rsi', {}).get('value', 0):.2f}
MACD Signal: {result.get('technical', {}).get('momentum', {}).get('macd', {}).get('signal', 'N/A')}
Overall Signal: {result.get('technical', {}).get('overall_summary', {}).get('signal', 'N/A')}

TRADING LEVELS
--------------
Entry Points:
{chr(10).join([f"  - ${entry.get('price', 0):.2f} ({entry.get('position_size', 'N/A')})" for entry in result.get('trading_levels', {}).get('entry_points', [])])}

Take Profit:
{chr(10).join([f"  - ${target.get('price', 0):.2f} ({target.get('probability', 'N/A')})" for target in result.get('trading_levels', {}).get('take_profit_targets', [])])}

Stop Loss: ${result.get('trading_levels', {}).get('stop_loss', {}).get('initial', {}).get('price', 0):.2f}

RECOMMENDED STRATEGY
--------------------
Primary: {result.get('strategies', {}).get('primary_recommendation', {}).get('strategy', 'N/A')}
Action: {result.get('strategies', {}).get('primary_recommendation', {}).get('action', 'N/A')}
Confidence: {result.get('strategies', {}).get('primary_recommendation', {}).get('confidence', 'N/A')}

Risk Warnings:
{chr(10).join([f"  - {warning}" for warning in result.get('strategies', {}).get('warnings', [])])}

---
Generated by Master Analysis v2.0
"""
